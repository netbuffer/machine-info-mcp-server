...
一个轻量级的主机信息获取MCP工具，通过MCP（模型上下文协议）接口提供系统信息，基于Spring Boot 4.1.1 + Spring AI 2.0.1 + OSHI（操作系统和硬件信息）库构建，并启用JDK 21虚拟线程以提升并发性能。
...
```json
{
  "mcpServers": {
    "machine-info-mcp-server": {
      ...
      "env": {
        "JAVA_HOME": "/path/to/your/java21"  // 可选，如果系统未设置JAVA_HOME
      },
